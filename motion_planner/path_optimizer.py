...
class PathOptimizer:
    def __init__(self):
        ...
        self._yf = 0.0
        self._tf = 0.0
        self._end_point_curvature = 0.00  #Default: 0.0; shall not exceed 0.05;
        self._onethird_max_curvature = 0.9  #Default: 0.5;
        self._twothird_max_curvature = 0.9  #Default: 0.5;

    def optimize_spiral(self, xf, yf, tf):
        """Optimization function used for finding the optimization parameters
        solving a spiral to given goal point (xf,yf, tf).
        Assumptions:
        ...
        p0 = [self._end_point_curvature, self._end_point_curvature, sf_0]
        ...
        bounds = [[-self._onethird_max_curvature, self._onethird_max_curvature],
                  [-self._twothird_max_curvature, self._twothird_max_curvature],
                  [sf_0, np.inf]]
        ...
        # self.objective_grad. The bounds are computed above, and the initial
        # variables for the optimizer are set by p0. L-BFGS-B method is used.

        res = scipy.optimize.minimize(self.objective, p0,
    ...
    def thetaf(self, a, b, c, d, s):
        ...
        Equation of a cubic spiral is kappa(s) = a + b*s + c*s^2 + d*s^3
        Theta(s) is the integral of kappa(s) with respect tovarc length
        then theta(s) = a*s + b/2*s^2 + c/3*s^3 + d/4*s^4.
        :param:
            ...
            s: list - spline parameter.
        :return: theta : list - theta values along the curve.
        """

        theta = np.add(np.add(np.add(np.multiply(a, s), np.multiply(b/2, np.power(s, 2))),
                              np.multiply(c/3, np.power(s, 3))), np.multiply(d/4, np.power(s, 4)))

        return theta

    def sample_spiral(self, p):
        """Samples a set of points along the spiral given the optimization
        parameters as [p1, p2, sf]
        ...
        # Uses self.thetaf() to compute the theta values from the s values.
